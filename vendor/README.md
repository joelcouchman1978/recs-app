...
- Place the pnpm store in `vendor/pnpm-store` (run `pnpm fetch` on a connected machine, archive the resulting store, and extract it here). After copying, run `PNPM_STORE_DIR="$(pwd)/../../vendor/pnpm-store" pnpm install --offline` in `apps/web`.
